...
| MVP-021 | MVP+ | File browser repair, footer health bar, icons, and exclusion regex rules | Implemented | PR #13; `src/FluxVault.App/ViewModels/FileBrowserViewModel.cs`; `src/FluxVault.App/MainWindow.xaml`; `src/FluxVault.Core/Configuration/ProtectionExclusionMatcher.cs` | `tests/FluxVault.App.Tests/FileBrowserViewModelTests.cs`; `MainWindowViewModelRefreshTests.cs`; `XamlQualityTests.cs`; `tests/FluxVault.Core.Tests/ProtectionExclusionRuleTests.cs`; `tests/FluxVault.Integration.Tests/ServiceOperationsTests.cs` | Preserves unsaved browser edits across refresh, adds discard/reload, applies path regex exclusions before capture, and moves health tiles to the footer. |
...
| V1-003 | V1 | Restore browser hardening and Explorer entry points | Implemented | PR TBD; `src/FluxVault.App/Services/RestoreServices.cs`; `src/FluxVault.App/Services/ExplorerContextMenuService.cs`; `src/FluxVault.App/Services/AppStartupRequestRouter.cs`; `src/FluxVault.App/ViewModels/MainWindowViewModel.cs`; `src/FluxVault.App/ViewModels/OptionsViewModel.cs`; `src/FluxVault.Core/Service/FluxVaultOperations.cs` | `tests/FluxVault.App.Tests/MainWindowViewModelRefreshTests.cs`; `tests/FluxVault.App.Tests/AppStartupRequestRouterTests.cs`; `tests/FluxVault.App.Tests/OptionsViewModelTests.cs`; `tests/FluxVault.App.Tests/XamlQualityTests.cs`; `tests/FluxVault.Integration.Tests/ServiceOperationsTests.cs`; `tests/FluxVault.Integration.Tests/PackagingTests.cs` | Restore now requires destination selection, confirms overwrites before IPC, preserves selection on cancellation/failure, reports restore failures safely, and adds Options-managed HKCU Explorer file/folder actions with `--restore-path` handoff to the running dashboard. Restore lineage metadata remains V1-004. |
